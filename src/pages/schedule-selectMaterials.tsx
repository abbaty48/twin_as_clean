...
import { useRecoilCallback, useRecoilState, useRecoilValue, useRecoilState_TRANSITION_SUPPORT_UNSTABLE } from "recoil"
...
import { PersonState } from "../recoil/atoms/scheduleAtom"
...
export const SelectItemsPhase: React.FC = () => {
   ...
   const setSchedule = useRecoilCallback(({ set }) => {
      return (newValue: any) => {
         set(PersonState, newValue)
      }
   })

   useEffect(() => {
      setSchedule({ name: 'abbaty', age: 30 })
   }, [])
   ...
   const addMoreMaterial = () => {
      /*  setSchedule((prevStates) => ({
          ...prevStates,
          unSelectedMaterials: [
             ...materials.filter(_m => prevStates.selectedMaterials.find(_s => _s.id !== _m.id))
          ],
          selectedMaterials: [
             ...prevStates.selectedMaterials,
             { ...prevStates.unSelectedMaterials[0] }
          ],
       })) */
   }
